...
dat<-read.csv("Z:/MHFD/First Creek/888_SCRIPTS/ContributingDrainageData/SWMM_Conduits.csv",header = FALSE,fileEncoding="UTF-8-BOM")
...
sort_dp<-matrix(0,nrow=length(design_p$val),ncol=18)
...
sort$area_contributed_to_DP<-round(sort$area_contributed_to_DP,digits=0)
sort$area_from_basins<-round(sort$area_from_basins,digits=3)
write.csv(sort,"Z:/MHFD/First Creek/888_SCRIPTS/ContributingDrainageData/FCMainDP_AreaCalcs.csv")
